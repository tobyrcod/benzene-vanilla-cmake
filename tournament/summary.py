    ...
    with open(fname, "r") as f:
        ...
        while line:
            if line[0] != "#":
                ...
                if longOpening:
                    opening = fullopening.strip()
                else:
                    moves = fullopening.strip().split(' ')
                    opening = moves[0]

                considerGame = True
                if validOpenings:
                    if opening not in validOpenings:
                        considerGame = False
                ...
                if not considerGame:
                    ...
                    if ((timeBlack > timeLimit and bres[0] == 'B') or
                        ...
                        overtime = timeBlack - timeLimit if bres[0] == 'B' else timeWhite - timeLimit
                    ...
                    if plotScore:
                        print(f"{p1Wins.count()}\t{p1Wins.mean()}\t{p1Wins.stderror()}", file=pf)
                    ...
                    for color in colors:
                        ...
                        key = (opening, color, who, 'win') if bres[0] == color else (opening, color, who, 'loss')
...
def usage():
    print(
        "Usage: ./summary [--count c] [--showTable] [--time max time] --openings [openings file] --file [tournament.result]")
    sys.exit(-1)
